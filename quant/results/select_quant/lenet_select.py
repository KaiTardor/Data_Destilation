...
if __name__ == '__main__':
    ...
    # Generar experimentos para fashion-mnist subsets (10% a 90%)
    base_train = Path("/mnt/homeGPU/haoweihu/quantize/dataset")
    
    methods   = ['phash', 'fps', 'kmeans']
    fractions = [i/10 for i in range(1, 10)]  # 0.1..0.9

    for idx, frac in enumerate(fractions, start=1):
        pct = int(frac*100)
        for method in methods:
            train_subset = base_train/f"{idx:02d}"/"fashion_mnist"/method
            #train_subset = base_train/f"{idx:02d}"/"cifar10"/method
            name = f"fmnist_{pct}pct_{method}_quant"
            #name = f"cifar_gray_{pct}pct_{method}_quant"
            #name = f"cifar_rgb_{pct}pct_{method}_quant"
